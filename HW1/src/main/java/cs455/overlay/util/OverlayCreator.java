...
public class OverlayCreator {
    ...
    public OverlayCreator(ArrayList<String> connections, int k) throws IllegalArgumentException {
        ...
        //TODO Verify that this does what it says it is going to do.
        Collections.shuffle(Arrays.asList(this.ipConnections));
        this.k = k;
        this.len = connections.size();
        this.fullOverlay = createOverlay();
    }

    public HashMap<String, ArrayList<String>> getFullOverlay() {
        return (HashMap<String, ArrayList<String>>) this.fullOverlay.clone();
    }

    public HashMap<String, ArrayList<String>> getRmvDuplicatesOverlay() {
        this.rmvDuplicatesOverlay = (HashMap<String, ArrayList<String>>) this.fullOverlay.clone();
        for(String key : fullOverlay.keySet()){
            for(String peerNode : fullOverlay.get(key)){
                if(!rmvDuplicatesOverlay.remove(peerNode, key)){
                    System.out.println("Removed " + peerNode + " since all of the nodes have already been connected");
                    rmvDuplicatesOverlay.remove(peerNode);
                }
            }
        }
        return rmvDuplicatesOverlay;
    }
